...
class SingleLine(Tbpu):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.tbpuName: str = "排版解析-单栏-单行"

    def get_lines(self, textBlocks: TextBlocks) -> List[List[dict]]:
        """
        从文本块列表中找出所有行
        ...
        for i1, tb1 in enumerate(textBlocks):
            ...
            for i2 in range(i1 + 1, len(textBlocks)):
                ...
                # 符合条件
                now_line.append(tb2)
                textBlocks[i2] = None  # type: ignore
                # 更新搜索条件
                r1 = r2
            ...
            for i2 in range(len(now_line) - 1):
                ...
                h = (b1 + b2 - t1 - l2) * 0.5
                if l2 - r1_box > h * 1.5:  # 间隙太大，强制设置空格
                    now_line[i2]["end"] = " "
                    continue
            ...
            textBlocks[i1] = None  # type: ignore
    ...
    def run(self, textBlocks: TextBlocks) -> TextBlocks:
        ...
        try:
            ...
            if not isinstance(textBlocks, list):
                logger.warning(f"SingleLine: 输入类型错误: {type(textBlocks)}，期望 list")
                return []
