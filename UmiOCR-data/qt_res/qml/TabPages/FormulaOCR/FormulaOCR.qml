...
TabPage {
    id: tabPage
    // 配置
    configsComp: FormulaOcrConfigs {}
    property string msnState: "none" // OCR任务状态， none run
    property string currentLatex: "" // 当前识别的LaTeX代码
    ...
    function copyLatex() {
        if(currentLatex !== "") {
            const delimiter = configsComp.getValue("output.delimiter")
            let text = currentLatex
            if(delimiter === "inline") {
                text = "$" + text + "$"
            } else if(delimiter === "display") {
                text = "$$" + text + "$$"
            }
            qmlapp.utilsConnector.copyText(text)
            qmlapp.popup.simple(qsTr("已复制LaTeX代码"), text)
        }
    }
    ...
    // 订阅事件
    function eventSub() {
        qmlapp.pubSub.subscribeGroup("<<formulaReScreenshot>>", this, "reScreenshot", ctrlKey)
        qmlapp.pubSub.subscribeGroup("<<formulaScreenshot>>", this, "screenshot", ctrlKey)
        qmlapp.pubSub.subscribeGroup("<<formulaPaste>>", this, "paste", ctrlKey)
        qmlapp.systemTray.addMenuItem("<<formulaScreenshot>>", qsTr("屏幕功能"), screenshot)
        qmlapp.systemTray.addMenuItem("<<formulaPaste>>", qsTr("粘贴图片"), paste)
    }
    // 取消订阅事件
    function eventUnsub() {
        qmlapp.pubSub.unsubscribeGroup(ctrlKey)
        qmlapp.systemTray.delMenuItem("<<formulaScreenshot>>")
        qmlapp.systemTray.delMenuItem("<<formulaPaste>>")
    }
    ...
    function onOcrGet(res, imgID="", imgPath="") {
        ...
        if(res.code === 100 && res.data && res.data.length > 0) {
            currentLatex = res.data[0].latex || res.data[0].text || ""
            latexPreview.text = currentLatex
            // 更新置信度显示
            if(res.data[0].score !== undefined) {
                confidenceText.text = qsTr("置信度: ") + (res.data[0].score * 100).toFixed(1) + "%"
            }
        } else {
            currentLatex = ""
            latexPreview.text = ""
    ...
    function onOcrEnd(msg) {
        if(msg.startsWith("[Error]")) {
            qmlapp.popup.message(qsTr("公式识别任务异常"), msg, "error")
        }
    }
    ...
    function showSimple(res, resText, isCopy) {
        ...
        if(code === 100) {
            if(isCopy) title = qsTr("已复制LaTeX代码")
            else title = qsTr("公式识别完成")
        }
        else if(code === 101) {
            title = qsTr("未检测到公式")
            resText = ""
                    ...
                    Column {
                        ...
                        Item {
                            ...
                            Row {
                                ...
                                Button_ {
                                    text_: qsTr("复制LaTeX")
                                    toolTip: qsTr("复制LaTeX代码到剪贴板")
                                    onClicked: tabPage.copyLatex()
                                    anchors.top: parent.top
                                    anchors.bottom: parent.bottom
                                }
                            }
                        ...
                        Rectangle {
                            width: parent.width
                            height: parent.height - size_.line * 1.5 - size_.smallSpacing
                            color: theme.bgColor
                            border.width: 1
                            ...
                            ScrollView {
                                ...
                                TextEdit_ {
                                    ...
                                    onTextChanged: {
                                        // 手动编辑时同步更新currentLatex
                                        tabPage.currentLatex = text
                                    }
                                }
                            }
                        }
